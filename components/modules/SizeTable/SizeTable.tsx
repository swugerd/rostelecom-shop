...
import s from '@/styles/size-table/index.module.scss'
...
const SizeTable = () => {
  ...
  const [xxlSize, setXXLize] = useState(false)
  ...
  const trProps = (
    ...
    style: {
      backgroundColor:
        item.isSelected ||
        selectedSize === item.manufacturerSize.toLocaleLowerCase()
          ? '#9466FF'
          : 'transparent',
  ...
  return (
    <div
      className={`${s.size_table} ${isHeaddressType ? s.size_table_headdress : ''}`}
    >
      <button
        className={`btn-reset ${s.size_table__close}`}
        onClick={handleCloseSizeTable}
      />
      <h2 className={s.size_table__title}>
        {translations[lang].size_table.title}
      </h2>
      <div className={s.size_table__inner}>
        <table className={s.size_table__table}>
          <thead>
            {isHeaddressType ? (
              <tr>
          ...
          <tbody>
            {isHeaddressType
              ...
              : dressSizes.map((item) => (
                  <tr
                    key={item.id}
                    {...(trProps(item) as HTMLAttributes<HTMLTableRowElement>)}
                  >
      ...
      <AddToCartBtn
        className={`${s.size_table__btn} ${s.size_table__btn_favorite}`}
